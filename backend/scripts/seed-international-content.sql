...
-- Run on VPS: docker compose exec -T postgres psql -U store -d store < backend/scripts/seed-international-content.sql
...
INSERT INTO settings (id, key, value, category, created_at, updated_at) VALUES
  ...
  (gen_random_uuid(), 'brand_story_content', '"In a small workshop in Jaipur, a group of women sit together each morning and begin to stitch. No machines. No rush. Just thread, needle, and years of knowledge passed down from mothers and grandmothers. That''s where every Store piece begins."', 'content', now(), now()),
  ...
  (gen_random_uuid(), 'storefront_url', '"http://localhost:3100"', 'general', now(), now())
...
<p>At Store, our artisans use cotton thread on cotton fabric. It takes between 4 and 14 days to complete a single piece, depending on its size and complexity. We don't rush it.</p>
...
<h2>The Store pieces</h2>

<p>Every Store piece uses Kantha stitch or block-print technique — or both. The women who make them have been doing this for decades. Their names are on the care label inside every piece we ship.</p>
...
$POST$,
  ...
  'published',
  now(),
  'What is Kantha? India''s 300-Year-Old Embroidery Tradition — Store',
...
VALUES
  ...
  (gen_random_uuid(), 'Priya N.', 'I bought a block-print jacket for my mother''s birthday and she cried when she opened it. The craftsmanship is exceptional. Store is the real thing — not the mass-produced "handmade" you see everywhere else.', 5, 'Toronto, Canada', true, now(), now())
